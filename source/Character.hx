...
class Character extends FlxSprite
{
	...
	public function new(x:Float, y:Float, ?character:String = "bf", ?isPlayer:Bool = false)
	{
		...
		if(FlxG.save.data.antialiasing) antialiasing = true;
		...
		switch (curCharacter)
		{
			...
			case 'default':
				...
				addOffset('idle', 0, 37);
				addOffset("singUP", -10, 87);
				addOffset("singRIGHT", -72, 40);
				addOffset("singLEFT", 65, 50);
				addOffset("singDOWN", 220, -20);

				playAnim('idle');

				flipX = true;
			case 'niko':
				tex = Paths.getSparrowAtlas('characters/niko','shared');
				frames = tex;
				animation.addByPrefix('idle', 'niko niko idle', 24, false);
				animation.addByPrefix('singUP', 'niko niko up', 24, false);
				animation.addByPrefix('singDOWN', 'niko niko down', 24, false);
				animation.addByPrefix('singLEFT', 'niko niko left', 24, false);
				animation.addByPrefix('singRIGHT', 'niko niko right', 24, false);
			...
			if (!curCharacter.startsWith('gf')) {
				if (!curCharacter.startsWith('bf')) {
					if (animation.curAnim.name.startsWith('sing')) {
						if (wasIdle) holdTimer = 0;
						holdTimer += elapsed;
						if (holdTimer >= Conductor.stepCrochet * 4 * 0.001 && animation.curAnim.finished) dance(true);
						wasIdle = false;
					} else {
						if (!wasIdle) holdTimer = 0;
						holdTimer += elapsed;
						if (holdTimer >= Conductor.stepCrochet * 4 * dadVar * 0.001) {
							dance();
							holdTimer = 0;
						}
						wasIdle = true;
