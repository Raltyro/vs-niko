...
class ChartingState extends MusicBeatState
{
	...
	public static var wasOnEditor = false;
	...
	**/
	...
	{
		#if windows
		if (ChartingState.wasOnEditor != true) DiscordClient.changePresence("Chart Editor", null, null, true);
		#end
		ChartingState.wasOnEditor = false;
	...
	function addSongUI():Void
	{
		...
		var saveCleanButton:FlxButton = new FlxButton(saveButton.x, saveButton.y + 30, "Save -Clean", function()
		{
			saveLevelClean();
		});
		...
		var cleanUpButton:FlxButton = new FlxButton(reloadSongJson.x, reloadSongJson.y + 60, "Clean up", function()
		{
			cleanUpSong();
		});
		...
		tab_group_song.add(saveCleanButton);
		tab_group_song.add(reloadSong);
		tab_group_song.add(cleanUpButton);
	...
	var check_mirrorSection:FlxUICheckBox;
	var check_mirrorSection2:FlxUICheckBox;
	...
	function addSectionUI():Void
	{
		...
		var mirrorSection:FlxButton = new FlxButton(10, 190, "Mirror Section", function()
		{
			for (i in 0..._song.notes[curSection].sectionNotes.length)
			{
				...
				// im lazy :/
				if (((note[1] == 1 || note[1] == 2 || note[1] == 5 || note[1] == 6) && check_mirrorSection2.checked != true) != true) {
					if (note[1] > 3) {
						if (check_mirrorSection.checked != true) {
							note[1] = 7+(note[1]-4)*-1;
						}
					} else {
						if (check_mirrorSection.checked == true) {
		...
		check_mirrorSection = new FlxUICheckBox(mirrorSection.x + mirrorSection.width + 10, 190, null, null, 'Side', 30);
		check_mirrorSection.name = 'check_mirrorSection';
		check_mirrorSection.checked = true;
		
		check_mirrorSection2 = new FlxUICheckBox(mirrorSection.x + mirrorSection.width + 60, 190, null, null, 'Affects Up and Down', 130);
		check_mirrorSection2.name = 'check_mirrorSection2';
		check_mirrorSection2.checked = true;
		...
		tab_group_section.add(check_mirrorSection);
		tab_group_section.add(check_mirrorSection2);
		...
		if (!typingShit.hasFocus)
		{
			...
			if (!FlxG.keys.pressed.SHIFT)
			{
				if (FlxG.keys.pressed.W || FlxG.keys.pressed.S)
				{
					...
					{
						FlxG.sound.music.time -= daTime * 3;
					}
					else
						FlxG.sound.music.time += daTime * 3;
	...
	function cleanUpSong():Void
	{
		var json = {
			"song": _song
		};
		var newjson:Jsonsaveformat;
		
		if (_song.notes != null && _song.notes.length > 0) {
			trace('line 1498');
			var bpm:Null<Float> = _song.bpm;
			var prevbpm:Null<Float> = bpm;
			var prevaltanim:Null<Bool> = (_song.notes[0].altAnim) ? true : false;
			var notes:Array<Dynamic> = [];
			var sections = [];
			var loopwithoutnothing:Int = 0;
			for (i in 0..._song.notes.length) {
				if (_song.notes[i].changeBPM)
				{bpm = json.song.notes[i].bpm;}
				else {json.song.notes[i].bpm = bpm;}
					
				if (_song.notes[i].sectionNotes != null) {
					for (v in _song.notes[i].sectionNotes) {
						notes.push([Math.round((Math.round(v[0]) / (Conductor.stepCrochet / 6)) * (Conductor.stepCrochet / 6)),Math.floor(v[1]),Math.floor(v[2]),json.song.notes[i].mustHitSection]);
					}
				}
				
				if (i == 0) {
					json.song.notes[i].changeBPM = json.song.notes[i].changeBPM ? true : null;
					json.song.notes[i].bpm = json.song.notes[i].changeBPM ? bpm : null;
					json.song.notes[i].altAnim = json.song.notes[i].altAnim ? true : false;
				} else {
					if (json.song.notes[i].bpm != null && json.song.notes[i].bpm == prevbpm) {json.song.notes[i].changeBPM = null; json.song.notes[i].bpm = null;}
					if (json.song.notes[i].altAnim == false && prevaltanim == false) {json.song.notes[i].altAnim = null;}
				}
				
				prevbpm = bpm;
				prevaltanim = json.song.notes[i].altAnim ? true : false;
				
				var section:Map<String,Dynamic> = new Map<String,Dynamic>();
				section.set('lengthInSteps',json.song.notes[i].lengthInSteps);
				section.set('mustHitSection',json.song.notes[i].mustHitSection);
				section.set('sectionNotes',[]);
				section.set('typeOfSection',json.song.notes[i].typeOfSection);
				if (json.song.notes[i].changeBPM) section.set('bpm',json.song.notes[i].bpm);
				if (json.song.notes[i].changeBPM) section.set('changeBPM',json.song.notes[i].changeBPM);
				if (json.song.notes[i].altAnim != null) section.set('altAnim',json.song.notes[i].altAnim);
			...
			var temp3:Bool = true;
			bpm = _song.bpm;
			var bpm2:Null<Float> = (sections[1] != null && sections[1].get('bpm') != null) ? sections[1].get('bpm') : bpm;
			var bpm3:Null<Float> = (sections[2] != null && sections[2].get('bpm') != null) ? sections[2].get('bpm') : bpm2;
			var prevnote = [];
			var i:Int = 0;
			for (v in notes) {
				temp3 = true;
				if (i > 0) {
					if (prevnote[0] == v[0] && prevnote[1] == v[1]) {
						if (prevnote[2] > v[2]) {temp3 = false;} else {sections[section].get('sectionNotes').remove(sections[section].get('sectionNotes')[i]);}
					}
				}
				if (temp3) {
					i += 1;
					for (i in section...sections.length) {
						if (time > v[0]) {
						//	while (time > v[0]) {
						//		if (v[0] > time) {break;}
						//		time = time - (60000/((sections[section] != null && sections[section].get('bpm') != null) ? sections[section].get('bpm') : bpm));
						//		section = section - 1;
						//		trace("backwards - " + time + " - " + v[0] + " - " + section + " - " + ((60000 / bpm) * ((_song.notes[section].lengthInSteps != null ? _song.notes[section].lengthInSteps : 16) / 16)));
						//	}
						//	section = section + 1;
							break;
						} else {
						...
						bpm = sections[section].get('bpm') != null ? sections[section].get('bpm') : bpm;
						bpm2 = (sections[section+1] != null && sections[section+1].get('bpm') != null) ? sections[section+1].get('bpm') : bpm2;
						bpm3 = (sections[section+2] != null && sections[section+2].get('bpm') != null) ? sections[section+2].get('bpm') : bpm3;
						time = time + ((60000/bpm) * ((_song.notes[section].lengthInSteps != null ? _song.notes[section].lengthInSteps : 16) / 4));
						trace(time + " - " + v[0] + " - " + section + " - " + ((60000 / bpm) * ((_song.notes[section].lengthInSteps != null ? _song.notes[section].lengthInSteps : 16) / 4)));
					}
					if (sections.length < section) {
						while (sections.length < section) {
							if (sections.length > section) break;
							var section:Map<String,Dynamic> = new Map<String,Dynamic>();
							section.set('lengthInSteps',sections[sections.length].get('lengthInSteps'));
							section.set('mustHitSection',sections[sections.length].get('mustHitSection'));
							section.set('sectionNotes',[]);
							section.set('typeOfSection',sections[sections.length].get('typeOfSection'));
					...
					temp = v[1];
					if (sections[section].get('mustHitSection') != v[3]) {
						if (v[1] > 3) {temp = temp - 4;} else {temp = temp + 4;}
					}
					sections[section].get('sectionNotes').push([v[0],temp,v[2]]);
					trace('line 1573');
					
					temp2 = temp2 + 1;
					if (temp2 > 2000) {temp2 = 0; Sys.sleep(3);}
					
					prevnote = v;
				}
			}
			
			var prevlis:Int = sections[0].get('lengthInSteps');
			var prevmhs:Bool = sections[0].get('mustHitSection');
			trace('line 1577');
			for (i in 0...sections.length) {
				loopwithoutnothing = loopwithoutnothing + 1;
				if (sections[i].get('changeBPM') || sections[i].get('lengthInSteps') != prevlis || sections[i].get('sectionNotes').length > 0 || sections[i].get('mustHitSection') != prevmhs) loopwithoutnothing = 0;
				prevlis = sections[i].get('lengthInSteps') != null ? sections[i].get('lengthInSteps') : 16;
				trace('line 1582');
			}
			trace('line 1584');
			loopwithoutnothing = loopwithoutnothing - 1;
			if (loopwithoutnothing < 0) loopwithoutnothing = 0;
			if (loopwithoutnothing > 0) {
				section = sections.length-loopwithoutnothing;
				for (i in sections.length-loopwithoutnothing...sections.length) {
					sections.remove(sections[section]);
					trace('line 1591');
				}
			}
			
			trace('line 1595');
			var newsong:Map<String,Dynamic> = new Map<String,Dynamic>();
			newsong.set('song',_song.song);
			newsong.set('notes',sections);
			newsong.set('bpm',_song.bpm);
			newsong.set('needsVoices',_song.needsVoices);
			newsong.set('speed',_song.speed);
			newsong.set('player1',_song.player1);
			newsong.set('player2',_song.player2);
			newsong.set('validScore',_song.validScore);
			if (_song.gfVersion != null && _song.gfVersion != 'gf') newsong.set('gfVersion',_song.gfVersion);
			if (_song.noteStyle != null && _song.noteStyle != 'normal') newsong.set('noteStyle',_song.noteStyle);
			if (_song.stage != null && _song.stage != 'stage') newsong.set('stage',_song.stage);
		...
		PlayState.SONG = Song.parseJSONshit(Json.stringify(newjson));
		ChartingState.wasOnEditor = true;
		LoadingState.loadAndSwitchState(new ChartingState());
	}
	
	private function saveLevelClean()
	{
		var json = {
			"song": _song
		};
		...
		var data:String = Json.stringify(newjson);
