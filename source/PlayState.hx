...
class PlayState extends MusicBeatState
{
	...
	public static var PREVSONG:String = '';
	...
	var vignette:Null<FlxSprite>;
	var vignette2:Null<FlxSprite>;
	...
	override public function create()
	{
		...
		{
			case 'blue-phosphor':
				dialogue = CoolUtil.coolTextFile(Paths.txt('data/blue-phosphor/blue-phosphorDialogue'));
			case 'amber':
				dialogue = CoolUtil.coolTextFile(Paths.txt('data/amber/amberDialogue'));
		}
		...
		if (SONG.stage == null)
		{
			...
			{
				case 0:
					if (songLowercase == 'blue phosphor') stageCheck = 'barrens';
					if (songLowercase == 'refuge') stageCheck = 'refuge';
				/*case 2:
		...
		if (!PlayStateChangeables.Optimize)
		{
			switch (stageCheck)
			{
				case 'refuge' | 'barrens' | 'amber':
					{
						defaultCamZoom = 0.9;
						curStage = stageCheck;
						vignette = new FlxSprite(FlxG.width * .5,FlxG.height * .5);
						vignette.loadGraphic(Paths.image('extras/vignette','shared'));
						vignette.alpha = .5;
						vignette.setGraphicSize(FlxG.width * 2);
						
						vignette2 = new FlxSprite(FlxG.width * .5,FlxG.height * .5);
						vignette2.loadGraphic(Paths.image('extras/vignette','shared'));
						vignette2.alpha = .5;
						vignette2.setGraphicSize(FlxG.width * 2);
						
						vignette.cameras = [camHUD];
						vignette2.cameras = [camHUD];
						if (FlxG.save.data.antialiasing) {vignette.antialiasing = true;vignette2.antialiasing = true;}
						
						if (stageCheck == 'barrens') {
							var bg:FlxSprite = new FlxSprite(-FlxG.width * .5,-FlxG.height * .5).loadGraphic(Paths.image('extras/stages/barrens/bg','shared'));
							bg.setGraphicSize(2200);
							bg.active = false;
							bg.scrollFactor.set(.75,.75);
							
							var ground:FlxSprite = new FlxSprite(-FlxG.width * .5,-FlxG.height * .5).loadGraphic(Paths.image('extras/stages/barrens/ground','shared'));
							ground.setGraphicSize(2300);
							ground.active = false;
							ground.scrollFactor.set(.9,.9);
							
							var pool1:FlxSprite = new FlxSprite(-FlxG.width * .5,-FlxG.height * .5).loadGraphic(Paths.image('extras/stages/barrens/pool1','shared'));
							pool1.setGraphicSize(2300);
							pool1.active = false;
							pool1.scrollFactor.set(.925,.925);
							
							var pool2:FlxSprite = new FlxSprite(-FlxG.width * .5,-FlxG.height * .5).loadGraphic(Paths.image('extras/stages/barrens/pool2','shared'));
							pool2.setGraphicSize(2300);
							pool2.active = false;
							pool2.scrollFactor.set(.925,.925);
							
							var terminal:FlxSprite = new FlxSprite(-FlxG.width * .5,-FlxG.height * .5).loadGraphic(Paths.image('extras/stages/barrens/terminal','shared'));
							terminal.setGraphicSize(2500);
							terminal.active = false;
							terminal.scrollFactor.set(.925,.925);
							
							var groundfront:FlxSprite = new FlxSprite(-FlxG.width * .5,-FlxG.height * .5).loadGraphic(Paths.image('extras/stages/barrens/groundfront','shared'));
							groundfront.setGraphicSize(2500);
							groundfront.active = false;
							groundfront.scrollFactor.set(1,1);
							...
							if (FlxG.save.data.antialiasing) {
								...
								pool1.antialiasing = true;
								pool2.antialiasing = true;
								terminal.antialiasing = true;
		...
		if (isStoryMode)
		{
			switch (StringTools.replace(curSong, " ", "-").toLowerCase())
			{
				...
				case 'thorns':
					schoolIntro(doof);
				case 'blue-phosphor' | 'amber' | 'refuge': 
					if (doof != null) {
						inCutscene = true;
						add(doof);
					} else {
	...
	override function openSubState(SubState:FlxSubState)
	{
		if (paused)
		{
			...
			#if windows
			DiscordClient.changePresence((inResults ? "On Result Screen : " : "PAUSED on ")
				+ (inResults ? PREVSONG : SONG.song)
				+ " ("
				+ storyDifficultyText
				+ ") "
				+ Ratings.GenerateLetterRank(accuracy),
	...
	override public function update(elapsed:Float)
	{
		...
		if (generatedMusic && PlayState.SONG.notes[Std.int(curStep / 16)] != null)
		{
			...
			if (vignette2 != null) {
				var pos:Array<Float> = [FlxG.camera.scroll.x-(dad.getMidpoint().x+150),FlxG.camera.scroll.y-(dad.getMidpoint().y-100)];
				vignette2.alpha = (Math.abs(((Math.abs((pos[0]*pos[0])+(pos[1]*pos[1])) - 537834) / 117002) - 1) - .7) / 3.3;
				if (vignette2.alpha > 1) vignette2.alpha = 1;
				if (vignette2.alpha < 0) vignette2.alpha = 0;
			}
		...
		if (generatedMusic)
		{
			...
			notes.forEachAlive(function(daNote:Note)
			{
				...
				if (!daNote.mustPress && daNote.wasGoodHit)
				{
					...
					if (dad.curCharacter != 'gf') dad.holdTimer = 0;
	...
	function endSong():Void
	{
		...
		{
			PlayState.PREVSONG = PlayState.SONG.song;
			...
			if (isStoryMode)
			{
				...
				if (storyPlaylist.length <= 0)
				{
					...
					PlayState.SONG.song = 'end'+storyWeek;
