...
class Main extends Sprite {
	...
	public static var timer:RaltTimer;
	...
	private function setupGame():Void {
		...
		Application.current.window.onClose.add(function() {
			if (!exited) {Application.current.window.onClose.cancel();} else System.exit(0);
			if (allowExit) {
				Main.exiting = true;
				FlxTween.tween(FlxG.sound.music,{volume : 0},1.5);
				new FlxTimer().start(1.725,function(tmr:FlxTimer) {Main.exited = true;System.exit(0);});
		...
		Application.current.onExit.add(function(exit:Int) {
