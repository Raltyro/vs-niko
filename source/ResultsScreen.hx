...
class ResultsScreen extends FlxSubState
{
	...
	{	
        ...
        settingsText = new FlxText(20,FlxG.height + 50,0,'SF: ${PlayState.rep.replay.sf} | Ratio (SA/GA): ${Math.round(sicks)}:1 ${Math.round(goods)}:1 | Mean: ${mean}ms | Played on ${PlayState.PREVSONG} ${CoolUtil.difficultyFromInt(PlayState.storyDifficulty).toUpperCase()}');
	...
	override function update(elapsed:Float) {
        ...
        if (PlayerSettings.player1.controls.ACCEPT)
        {
			...
			if (PlayState.isStoryMode) {
				if (PlayState.storyPlaylist.length <= 0) {
					var songHighscore = StringTools.replace(PlayState.PREVSONG, " ", "-");
        ...
        if (FlxG.keys.justPressed.F1 && !PlayState.loadRep)
        {
			...
			if (PlayState.isStoryMode)
			{
				PlayState.storyPlaylist.unshift(PlayState.PREVSONG);
        ...
        if (FlxG.keys.justPressed.F2  && !PlayState.loadRep)
        {
            ...
            var songFormat = StringTools.replace(PlayState.PREVSONG, " ", "-");
            switch (songFormat) {
                case 'Dad-Battle': songFormat = 'Dadbattle';
                case 'Philly-Nice': songFormat = 'Philly';
                case 'dad-battle': songFormat = 'Dadbattle';
            ...
            PlayState.SONG = Song.loadFromJson(poop, PlayState.PREVSONG);
